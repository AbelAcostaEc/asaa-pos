<x-pos-layout>
    ...
    <div x-data="userCrud()" class="space-y-6">
        ...
        <div class="hidden lg:block">
            <x-pos.card>
                <x-pos.table :headers="[
                    __('administration::users.col_id'),
                    __('administration::users.col_name'),
                    __('administration::users.col_email'),
                    __('administration::users.col_status'),
                    __('administration::users.col_updated'),
                    __('administration::users.col_actions'),
                ]">
                    @forelse($users as $user)
                        <tr class="transition-colors hover:bg-gray-50/50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <span class="{{ $user->is_active ? 'bg-success/10 text-success' : 'bg-danger/10 text-danger' }}
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-gray-400 dark:text-gray-500">
                                <svg class="mx-auto mb-3 h-12 w-12 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                    ...
                    @foreach ($users as $user)
                        <div class="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-white p-5
                            ...
                            <div class="mt-auto flex gap-2 border-t border-gray-100 pt-3 dark:border-gray-800">
                                ...
                                <button @click="confirmDisable({{ $user->id }}, {{ $user->is_active ? 'true' : 'false' }})"
                                        class="{{ $user->is_active ? 'bg-danger/10 text-danger hover:bg-danger/20' : 'bg-success/10 text-success hover:bg-success/20' }}
        ...
        <script>
            function userCrud() {
                return {
                    ...
                    openEditModal(user) {
                        this.editMode = true;
                        this.formData = { id: user.id, name: user.name, email: user.email, password: '' };
                        this.errors = {};
                        this.$dispatch('open-modal', 'user-modal');
                    },
